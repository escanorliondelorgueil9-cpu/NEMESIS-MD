...
async function connectToWhatsapp(handleMessage) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            try {
                const chatId = '237674400851@s.whatsapp.net'; // ton numéro ou le groupe cible
                ...
                const messageText = `
╔══════════════════╗
      *Nemesis-md Bot Connected Successfully* 🚀
╠══════════════════╣
> "Always Forward. Nemesis Md, one of the best."
╚══════════════════╝

*Nemesis je suis avide *
                `;
    ...
    setTimeout(async () => {
        if (!state.creds.registered) {
            ...
            try {
                ...
                const number = 237674400851; // mettez votre numéro WhatsApp 

                if (asPremium === true) {
                    configmanager.premiums.premiumUser['c'] = { creator: '237674400851' };
                    configmanager.saveP();
